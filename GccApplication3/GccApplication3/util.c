...
#warning "Unimplemented function: unsigned char read_push_buttons(void)"	// delete this line after implementing
	...
	// INSERT CODE HERE!
	char button;
	int bits = (PINC & 0b00111111);
	
	char myBits[6];
	for(int i =0; i < 6; i++)
	{
		myBits[i] = (char) ((bits >> i) & 1);		
	}
...
#warning "Unimplemented function: char read_shaft_encoder(void)"	// delete this line after implementing
	...
	static unsigned char old_value = 0b11;		// Step 2: Based on how you decided to mask PINC in step 1, what would new_value be if the switch is in a groove?
	
	// Function variables
	unsigned char new_value = (((PINC & 0b11000000) >> 6) & 11);			// Step 1: Decide how to read PINC so that the push buttons are masked
	char rotation = 0;

	// If the knob was in a groove
	if (old_value == 0b11) {			// Step 3: Enter here what you did for step 2.
		if (new_value == 0b01)		// What will new_value be if you're rotating CW out of a groove
			rotation = 1;
		if (new_value == 0b10)		// What will new_value be if you're rotating CCW out of a groove
			rotation = -1;
	}
